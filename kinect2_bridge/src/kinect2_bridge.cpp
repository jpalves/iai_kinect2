...
private:
  ...
  void receiveColor()
  {
       ...
       status[MONO_HD] || status[MONO_HD_RECT] || status[MONO_QHD] || status[MONO_QHD_RECT])
    {
      ...
      {
        cv::cvtColor(tmp, images[COLOR_HD], cv::COLOR_RGB2BGR);
      }
    }
  ...
  void processColor(std::vector<cv::Mat> &images, const std::vector<Status> &status)
  {
    ...
    if(status[MONO_HD])
    {
      cv::cvtColor(images[COLOR_HD], images[MONO_HD], cv::COLOR_RGB2GRAY);
    }
    if(status[MONO_HD_RECT])
    {
      cv::cvtColor(images[COLOR_HD_RECT], images[MONO_HD_RECT], cv::COLOR_RGB2GRAY);
    }
    if(status[MONO_QHD])
    {
      cv::cvtColor(images[COLOR_QHD], images[MONO_QHD], cv::COLOR_RGB2GRAY);
    }
    if(status[MONO_QHD_RECT])
    {
      cv::cvtColor(images[COLOR_QHD_RECT], images[MONO_QHD_RECT], cv::COLOR_RGB2GRAY);
    }
  }
